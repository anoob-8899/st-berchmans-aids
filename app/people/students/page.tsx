  ...
  const handleDeleteStudent = async (id: string, name: string) => {
    if (confirm(`Are you sure you want to delete profile for ${name}?`)) {
      try {
        ...
        if (res.ok) {
          setStudentsList(prev => prev.filter(s => s.id !== id && s.name !== name));
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-full text-[11px] font-bold text-rose-600 hover:text-white hover:bg-rose-600 bg-rose-50 border border-rose-200 transition flex items-center gap-1 cursor-pointer"
                      title="Delete Profile (Admin)"
                    >
